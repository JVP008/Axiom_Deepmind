...
const MENU_ITEMS: { id: ModuleId; label: string; icon: React.ReactNode; color: string }[] = [
  { id: 'dashboard', label: 'Dashboard', icon: <LayoutDashboard size={20} />, color: 'bg-neo-blue' },
  { id: 'timetable', label: 'Timetable', icon: <CalendarDays size={20} />, color: 'bg-neo-pink' },
  { id: 'attendance', label: 'Attendance', icon: <ClipboardCheck size={20} />, color: 'bg-neo-green' },
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#f0f4f8] dark:bg-slate-900">
      ...
      <aside 
        ...
        `}
      >
        <div className="h-16 flex items-center justify-between px-4 border-b-2 border-slate-900 dark:border-slate-100 bg-neo-yellow dark:bg-indigo-900">
        ...
        <div className="p-4 border-t-2 border-slate-900 dark:border-slate-100 bg-white dark:bg-slate-800">
          <div className="flex items-center justify-between">
            <div className={`flex items-center gap-3 overflow-hidden ${!isSidebarOpen && !isMobile ? 'hidden xl:flex' : 'flex'}`}>
                <div className="w-8 h-8 rounded-none border-2 border-slate-900 dark:border-slate-100 bg-neo-pink dark:bg-slate-700 flex items-center justify-center shadow-hard-sm">
                <span className="text-sm font-bold font-mono">AD</span>
                </div>
        ...
        <header className="h-16 flex items-center justify-between px-6 bg-white dark:bg-slate-900 border-b-2 border-slate-900 dark:border-slate-100 shrink-0 z-10">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-black uppercase tracking-tight flex items-center gap-2">
                <span className={`w-3 h-3 border-2 border-slate-900 dark:border-white ${MENU_ITEMS.find(m => m.id === currentModule)?.color}`}></span>
                {MENU_ITEMS.find(m => m.id === currentModule)?.label}
            </h1>
          </div>
        </header>
